...
import ru.Den_Abr.ChatGuard.Common.Commands.CommandHandler;
...
public class BukkitCommandHandler implements CommandHandler {
	...
	public void warnings(CGPlayer cs, String[] args) {
		CGPlayer player;
		if (args.length == 0) {
	...
	public void ban(CGPlayer cs, String[] args) {
	...
	public void unban(CGPlayer cs, String[] args) {
		String word = StringUtils.join(args, ' ').toLowerCase().trim();
	...
	public void whitelist(CGPlayer cs, String[] args) {
		String word = StringUtils.join(args, ' ').toLowerCase().trim();
		Whitelist.add(word);
		cs.sendMessage(Message.SUCCESSFULLY.get());
	}
	...
	public void cc(CGPlayer cs, String[] args) {
		if (args.length == 0) {
			if (cs instanceof Player) {
	...
	public void globalmute(CGPlayer cs, String[] args) {
		// ya
		ChatGuard.GLOBAL_MUTE = !ChatGuard.GLOBAL_MUTE;

		Bukkit.broadcastMessage(
				ChatGuard.GLOBAL_MUTE ? Message.GLOBAL_MUTE_ENABLED.get() : Message.GLOBAL_MUTE_DISABLED.get());
	}

	@Cmd(desc = "Mute player for some reason", name = "mute", perm = "chatguard.mute", args = "[Player] ([Time] (Reason))", min = 1, max = 3)
	public void mute(CGPlayer cs, String[] args) {
	...
	public void unMute(CGPlayer cs, String[] args) {
		Player pl = Bukkit.getPlayer(args[0]);
		if (pl == null) {
			cs.sendMessage(Message.PLAYER_NOT_FOUND.get());
			return;
	...
	public void clear(CGPlayer cs, String[] args) {
		if (args.length > 0) {
	...
	@Cmd(desc = "Clear messages of specified player", name = "of", perm = "chatguard.clearof", args = "[OF PLAYER]", min = 1)
	public void of(CGPlayer cs, String[] args) {
	...
	public void warn(CGPlayer cs, String[] args) {
		Player p = Bukkit.getPlayer(args[0]);
	...
	public void reload(CGPlayer cs, String[] args) {
	...
	public void help(CGPlayer cs, String[] args) {
		...
		for (SubCommand sc : CommandManager.getInstance().getSubCommands()) {
